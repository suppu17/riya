...
export interface CartModalRef {
  triggerCheckout: () => Promise<string>;
}

const CartModal = forwardRef<CartModalRef, CartModalProps>(
  ({ isOpen, onClose }, ref) => {
    ...
    const handleCheckout = async (): Promise<string> => {
      ...
      return `Thank you for your order! Order ID: ${newOrderId}. Your total is $${totalPrice.toFixed(
        2
      )} for ${totalItems} items. Your order has been confirmed and you'll receive a confirmation email shortly. Your cart will be cleared shortly.`;
    };

    // Expose the triggerCheckout method through ref
